      ...
      {models.length === 0 ? (
        ...
        <div className="flex flex-col gap-5">
          {models.map((m, i) => {
            ...
            return (
              <Card key={m.id} tilt={i % 2 ? 0.4 : -0.4}>
                ...
                <div className="flex flex-wrap items-center gap-2 mt-3">
                  <span className="text-sm text-ink/60 mr-1">ฮาร์ดแวร์</span>
                  {hardware.length > 0 ? (
                    hardware.map((h) => <Pill key={h}>{h}</Pill>)
                  ) : (
                    <span className="text-ink/45 text-sm">ยังไม่ได้ระบุ</span>
                  )}
                </div>
                ...
                {canEdit && (
                  ...
                  <details className="mt-4 group">
                    <summary className="cursor-pointer inline-flex items-center gap-2 border-2 border-ink wob-sm px-4 py-1 bg-muted shadow-hardSm select-none marker:content-none [&::-webkit-details-marker]:hidden hover:bg-pen hover:text-white transition-transform duration-100">
                    ...
                    <form
                      ...
                      className="grid gap-4 md:grid-cols-2 mt-4 border-2 border-dashed border-ink wob-sm p-4 bg-paper"
                    >
                      ...
                      <div className="md:col-span-2 flex flex-wrap items-center gap-3">
                        ...
                        <Link
                          href={`/models/${m.code}`}
                          className="underline decoration-wavy hover:text-accent"
                        >
                          ดูประวัติเวอร์ชัน
                        </Link>
                      </div>
